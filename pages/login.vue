    ...
    <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
          Access the dynamic portfolio CMS
        </p>
      </div>
      <form class="mt-8 space-y-6" @submit.prevent="handleLogin">
        ...
        <div v-if="errorMsg" class="text-sm font-medium text-red-600 text-center">
          {{ errorMsg }}
        </div>

        <div>
          <button type="submit" :disabled="isLoading" class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-gray-900 hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
...
definePageMeta({
  layout: false // Hide standard header/footer for login
})
